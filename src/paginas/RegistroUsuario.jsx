...
const Usuarios = () => {
  ...
  const atualizarCliente = async (id, name, cpf, tel) => {
    try {
      ...
      if (res.status === 200) {
        ...
        setUsuarios((prevUsuarios) =>
          prevUsuarios.map((user) =>
            user.id === id ? { ...user, name, cpf, tel } : user
          )
        );
      } else {
  ...
  const mostrarOpcoesWhatsApp = async (tel, id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await apiAcai.post("/int/wpp/welcome", { tel });
          if (res.status === 201) {
            Swal.fire({
              title: "Enviando...",
        ...
        <ContainerFlex>
                ...
                <tr key={usuario.cpf}>
                  ...
                  <td>
                    <FaWhatsappSquare
                      style={{ cursor: "pointer" }}
                      onClick={() =>
                        mostrarOpcoesWhatsApp(usuario.tel, usuario.id)
                      }
                    />
                  </td>
                </tr>
          ...
          >
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                atualizarCliente(id, name, cpf, tel);
              }}
            >
              <Form>
                <Form1>
